...
function MobileTierRow({ event, tier }: { event: EventProps; tier: TicketTier }) {
  return (
    <Link
      ...
      className="group flex items-center gap-3 rounded-2xl border-2 border-gray-100 bg-gray-50/80 p-4 transition hover:border-[#3b82f6] hover:bg-[#eef4ff]"
    >
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <ChevronRight className="h-5 w-5 text-gray-300 transition group-hover:text-[#3b82f6]" />
      </div>
    </Link>
  );
}
...
function DesktopTierCard({ event, tier }: { event: EventProps; tier: TicketTier }) {
  return (
    <Link
      href={`/events/${event._id}/checkout?tier=${tier.type}`}
      className="group flex h-full flex-col rounded-2xl border-2 border-gray-100 bg-gray-50/60 p-6 transition hover:border-[#3b82f6] hover:bg-[#eef4ff] hover:shadow-md"
    >
      ...
      <div className="mt-6 flex items-end justify-between gap-4 border-t border-dashed border-gray-200 pt-5">
        ...
        <span
          className={`${ticketFlowButtonInlineClassName} group-hover:brightness-110`}
